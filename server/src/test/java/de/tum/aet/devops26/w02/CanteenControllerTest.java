...
public class CanteenControllerTest {
  ...
  public void testGetTodayMeals_ReturnsNoContent_WhenNoMealsAvailable()
    ...
    // Act & Assert
    getList("/{canteenName}/today", HttpStatus.NO_CONTENT, Dish.class,
            "mensa-garching");
  }
  ...
  public void testGetTodayMeals_ReturnsOkWithMeals() throws Exception {
    ...
    String expectedUrl = "https://tum-dev.github.io/eat-api/" + canteenName +
                         "/" + year + "/" + weekStr + ".json";
    when(restTemplate.getForObject(eq(expectedUrl), eq(Week.class)))
        .thenReturn(expectedWeek);

    // Act
    List<Dish> actualTodayDishes =
        getList("/{canteenName}/today", HttpStatus.OK, Dish.class, canteenName);
  ...
  private <T> List<T> getList(String path, HttpStatus expectedStatus,
                              Class<T> listElementType, Object... uriVariables)
      throws Exception {
    MvcResult res =
        mockMvc
            .perform(
                get(path, uriVariables).contentType(MediaType.APPLICATION_JSON))
            .andExpect(status().is(expectedStatus.value()))
            .andReturn();

    if (expectedStatus.value() != 200) {
      if (res.getResponse().getContentType() != null &&
          !res.getResponse().getContentType().equals(
